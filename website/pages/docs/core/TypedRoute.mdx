...
Furthermore, as `TypedRoute` utilizes [`typia.assertStringify()`](https://typia.io/docs/json/stringify) function, such it is even type safe. 
...
```
    ...
    <Tab>
```javascript filename="BbsArticlesController.js" showLineNumbers {41-136}
"use strict";
...
__decorate([
    core_1.TypedRoute.Get("random", { 
        ...
        assert: input => { 
            ...
            const __is = input => {
                const $io0 = input => "string" === typeof input.id && $is_uuid(input.id) && "string" === typeof input.writer && ("string" === typeof input.title && 3 <= input.title.length && 50 >= input.title.length) && "string" === typeof input.body && ("string" === typeof input.created_at && $is_datetime(input.created_at)) && (Array.isArray(input.files) && input.files.every(elem => "object" === typeof elem && null !== elem && $io1(elem)));
        ...
        const stringify = input => {
            ...
            const $is_datetime = core_1.TypedRoute.Get.is_datetime;
            const $io1 = input => (null === input.name || "string" === typeof input.name) && (null === input.extension || "string" === typeof input.extension) && "string" === typeof input.url;
            const $so0 = input => `{"id":${"\"" + input.id + "\""},"writer":${$string(input.writer)},"title":${$string(input.title)},"body":${$string(input.body)},"created_at":${"\"" + input.created_at + "\""},"files":${`[${input.files.map(elem => $so1(elem)).join(",")}]`}}`;
