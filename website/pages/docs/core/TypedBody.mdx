...
```
    ...
    <Tab>
```javascript filename="BbsArticlesController.js" showLineNumbers {140-221}
"use strict";
...
## Comment Tags
You can enhance validation logic, of `@TypedBody()`, through comment tags.

You know what? `@TypedBody()` utilizes [`typia.assert<T>()`](https://typia.io/docs/validators/assert) function for request body data validation, and the [`typia.assert<T>()`](https://typia.io/docs/validators/assert) function supports additional type checking logics through comment tags. For reference, the "Comment Tag" means a comment starting from `@` symbol. It follows `@${name} ${value}` format, and you can add multiple comment tags per each property.
