...
Note that, never forget to defining the `@WebSocketRoute.Acceptor()` decorated parameter. It is essential for both WebSocket route method and SDK library generation. Each generic arguments of `WebAcceptor<Header, Provider, Listener>` means like below:
...
Also, if you don't want to accept the client connection, reject it through `WebAcceptor.close()` function.
...
## Restrictions
### `@WebSocketAcceptor()`
When defining `@WebSocketRoute()` decorated method, you must define the `@WebSocketRoute.Acceptor()` decorated parameter. It is essential for both WebSocket route method and SDK library generation, because its target type `WebAcceptor<Header, Provider, Listener>` has significant type definitions for WebSocket communication.
