...
<Tabs items={[
  ...
  <Tabs.Tab>
```typescript filename="src/controllers/shoppings/customers/ShoppingCustomerSaleController.ts" showLineNumbers {29, 43, 57}
import { TypedBody, TypedParam, TypedRoute } from "@nestia/core";
import { Controller } from "@nestjs/common";
import { tags } from "typia";
...
If you write `@hidden` or `@internal` comment tag on a controller method, it would be hidden in the Swagger Document. And if you write `@human` comment tag on a controller method, it would be shown in the Swwagger Document, but would be hidden in the LLM Application Schema.
