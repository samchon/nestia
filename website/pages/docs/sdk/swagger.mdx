...
```
  ...
  <Tab>
```typescript filename="INestiaConfig.ts" showLineNumbers {14-19, 167-240}
...
```
  ...
  <Tab>
```json filename="swagger.json" showLineNumbers {17-22, 26-28, 66-76, 81, 83, 128, 130-137}
{
  "openapi": "3.0.1",
  "servers": [
    {
...
If what you want is not just filling special properties of JSON schema spec, but to adding custom properties into the JSON schema definition, you can accomlish it with `typia` feature. Define a type based on `typia.tags.TagBase`, and specify the `schema` property type as you want.
