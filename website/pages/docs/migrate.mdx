...
Of course, even if you're not considering to adapt NestJS, you can generate standalone [SDK library](./sdk/sdk) and [Mockup Simulator](./sdk/simulator) from the `swagger.json` file. Just run the `@nestia/migrate` command and build the SDK library and Mockup Simulator, then frontend (client) developers may get satisfied.
