...
At first, look at the first (*`Triple duplicated NestJS DTO`*) tab, and find the `BbsArticle.files` property, enhanced by blue coloured blocks. Looking at the `files` property, how do you feel? Just defining an array object type, you've to call 5 decorator functions. If you take any mistake when using the decorator like omitting `isArray` property, it would be a critical runtime erorr.
...
<Tabs items={[
    ...
    <Tab>
```typescript filename="BbsArticle.ts" showLineNumbers {8-21}
...
```
    ...
    <Tab>
```javascript filename="BbsArticlesController.js" showLineNumbers {35-221}
"use strict";
