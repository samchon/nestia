---
title: "Reference · E2E Test Development"
description: "Build a production NestJS e2e test program on top of the generated SDK using @nestia/e2e and DynamicExecutor — discoverable functions, include/exclude filters."
---
...
If you've succeeded to generate [SDK library by `@nestia/sdk`](/docs/sdk), you can utilize the SDK library to implement e2e test functions. As the SDK library ensures types safety for remote API calls, you can develop much more efficient and safer test program than unit testing case.
...
async function main(): Promise<void> {
  ...
  const failures: DynamicExecutor.IExecution[] =
...
export async function test_api_bbs_article_create(
  ...
  const stored: IBbsArticle = await api.functional.bbs.articles.create(
    ...
    "general",
    {
      ...
      body: RandomGenerator.content({ paragraphs: 8 }),
      format: "txt",
      files: [
        {
          name: "logo",
