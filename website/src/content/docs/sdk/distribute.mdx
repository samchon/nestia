...
`nestia sdk` writes this `package.json` and `tsconfig.json` itself, installing the `@nestia/fetcher` of the running release and the typia, ttsc, and TypeScript your project uses, so the versions above only illustrate. `compile` runs `ttsc`, which applies typia's transform to the SDK's `assert` and `simulate` code. Most teams change `target` and `module` to match their consumers (ESM vs CJS, modern vs legacy browsers).
