...
```jsonc filename="packages/api/package.json" {2-3,9} copy
{
  ...
  "scripts": {
    ...
    "compile":   "rimraf lib && ttsc",
    "deploy":    "npm run build && npm publish"
  },
  "dependencies": {
    "@nestia/fetcher": "14.0.0",
    "typia":           "15.0.0"
  },
  "devDependencies": {
    "rimraf":     "^6.0.0",
...
`nestia sdk` writes this `package.json` and `tsconfig.json` itself, installing the `@nestia/fetcher` of the running release and the typia, ttsc, and TypeScript your project uses, plus its `tgrid` and `@modelcontextprotocol/sdk` when the SDK has WebSocket or MCP functions, so the versions above only illustrate. `compile` runs `ttsc`, which applies typia's transform to the SDK's `assert` and `simulate` code. Most teams change `target` and `module` to match their consumers (ESM vs CJS, modern vs legacy browsers).
