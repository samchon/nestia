...
The SDK generator writes the file tree at code-generation time. The `paths` alias never appears in the emitted `.js` / `.d.ts`, so no transform-paths pass or compiler patch is needed. Keep the `paths` entry purely for `tsc --noEmit` type-checking inside this workspace; published consumers resolve through the package's normal `exports` / `main`.
