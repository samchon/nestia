...
Previous Nestia versions required a patched TypeScript compiler (`ts-patch`) so that `typescript-transform-paths` could rewrite the `@samchon/shopping-api` alias into relative imports at emit time. In v7 the SDK generator writes the file tree at code-generation time — the `paths` alias never appears in the emitted `.js` / `.d.ts`, so no transform-paths pass and no compiler patch are needed. Keep the `paths` entry purely for `tsc --noEmit` type-checking inside this workspace; published consumers resolve through the package's normal `exports` / `main`.
