...
```json filename="packages/api/tsconfig.json" showLineNumbers {7,10-11}
{
  "compilerOptions": {
    ...
    "plugins": [{ "transform": "typia/lib/transform", "enabled": false }]
  },
  "include": ["src"]
}
```
...
Previous Nestia versions required a patched TypeScript compiler (`ts-patch`) so that `typescript-transform-paths` could rewrite the `@samchon/shopping-api` alias into relative imports at emit time. With the Go-backed transform, the SDK generator writes the file tree at code-generation time — the `paths` alias never appears in the emitted `.js` / `.d.ts`, so no transform-paths pass and no compiler patch are needed. Keep the `paths` entry purely for `tsc --noEmit` type-checking inside this workspace; published consumers resolve through the package's normal `exports` / `main`.
...
- [`packages/api/tsconfig.json`](https://github.com/samchon/shopping/blob/master/packages/api/tsconfig.json) — the self-alias under `paths`.
