...
description: Bootstrap a Nestia-ready NestJS project with `npx nestia start` or run the setup wizard against an existing codebase — covers npm, pnpm, and yarn classic.
---
...
During the setup process, the prompt will ask you to whether configure runtime swagger or not. If you hope to build [Swagger Document in the runtime](/docs/swagger/#runtime-composition), you have to choose `true` option. Otherwise, just select the `false` option please.
