...
`typia` must be the exact release your `@nestia/core` is built for. The Nestia transform is compiled from that typia release's Go source, while the code it emits calls the `typia` package installed in your project, so another version would mix two releases. `@nestia/core` depends on and peers that exact version, so npm refuses a different one at install, and when another is resolved anyway the build stops with an error naming both versions. `npm view @nestia/core dependencies.typia` prints the version to install.

NestJS must be 11 or newer, with `rxjs` 7.1 or newer, the versions Nestia builds and tests against. `@nestia/core` declares both as peer dependencies.
...
- **`multiple compiler native backends cannot share one emit pass`**: the plugin list names a second native host, such as nestia v11's `@nestia/core/lib/transform` on a release before it resolved to the native descriptor. Remove `compilerOptions.plugins`, or list only the disabled `typia` entry and `@nestia/core/native/transform.cjs` as shown above.
