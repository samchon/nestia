...
export function TypedException<T extends object>(props: {
  ...
  description?: string;
  example?: T;
  examples?: Record<string, TypedException.IExample<T>>;
}): MethodDecorator;
```
...
<Callout type="info">
**When you'd reach for this:** any route where 4xx / 5xx responses carry a structured body that the client cares about. Validation failures are not documented automatically: declare the 400 response with `@TypedException<T>(400)`, `T` being the body your validation errors carry, when clients should see it, and a `@TypedException` for each business error (`"order locked"`, `"insufficient funds"`).
</Callout>
