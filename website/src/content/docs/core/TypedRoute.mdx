...
description: Type-safe route decorators (`Get`, `Post`, `Put`, `Patch`, `Delete`) that validate response DTOs and serialize JSON up to 200x faster than `class-transformer`.
---
...
`@TypedRoute.Get()` is not essential for [Swagger Documents](/docs/swagger) or [SDK Library](/docs/sdk) building.
...
Additionally, if you turn on the `llm` property, `@nestia/core` validates at compile time that the route's return type stays within the schema subset LLM function calling accepts. The return type must be a **single, non-nullable, non-undefinable object**.
