...
<Callout type="info">
**New to Nestia?** Start with **[Tutorial → Your First Endpoint](/docs/tutorial/endpoint)** for the friendly walkthrough. This page is the reference.
</Callout>
...
See **[Concepts → Pure TypeScript Types](/docs/concepts/pure-typescript)** for what tags exist, and **[Concepts → AOT Compilation](/docs/concepts/aot)** for what the build-time transformer does.
...
`typia.json.stringify<T>` is up to **200× faster** than `class-transformer`. The full methodology and graphs live on **[Concepts → Why So Fast](/docs/concepts/why-fast)** — including the honest caveat that 200× is the peak, with 30–100× as the typical real-server speedup.
...
- **[Tutorial → Your First Endpoint](/docs/tutorial/endpoint)** — the introductory walkthrough.
- **[TypedBody](./TypedBody)** — type-safe `application/json` request bodies.
- **[TypedException](./TypedException)** — typed error responses.
- **[Concepts → AOT Compilation](/docs/concepts/aot)** — what the transformer is doing.
