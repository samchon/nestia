...
Encrypted routes appear in the Swagger document with a `text/plain` response, because the wire carries ciphertext; the response is marked `x-nestia-encrypted: true` and its description starts with a warning. Only the **generated SDK** (`@nestia/sdk`) can call them: it decrypts on the client side, so consumers see typed, decrypted data.
