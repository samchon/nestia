...
description: Type-safe path parameter decorator that infers the parameter type from the TypeScript signature and validates formats like `uuid` or `date` at request time.
---
...
`@TypedParam()` is not essential for [Swagger Documents](/docs/swagger) or [SDK Library](/docs/sdk) building.
...
Also, `@TypedParam()` allows nullable like `number | null`, but undefinable type is not.
