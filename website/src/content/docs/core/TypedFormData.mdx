...
<Callout type="info">
**Only `@TypedFormData.Body()` is supported by `@nestia/sdk`'s generators.** A route taking its payload through vanilla `@UploadedFile()` or `@UploadedFiles()` fails SDK and Swagger generation with a diagnostic: the multer field names live in the interceptor, where the generators cannot read them, so they could only describe a request without its files. Move the route to `@TypedFormData.Body()`, or tag the method `@ignore` to leave it out of the generators.
</Callout>
...
On Fastify, pass a [`fastify-multer`](https://www.npmjs.com/package/fastify-multer) instance instead:
