...
For the full tag catalog, see the [typia tags reference](https://typia.io/docs/validators/tags/).
...
`@nestia/sdk` reads a vanilla `@Body()` parameter too: the route appears in the generated SDK, the Swagger document, and the e2e suite with the declared body type. Only the runtime validation is skipped.
...
Encrypted bodies appear in the Swagger document as a `text/plain` request body, because the wire carries ciphertext; the body is marked `x-nestia-encrypted: true` and its description starts with a warning. Only the generated SDK can call them.

The ciphertext is a text body, so it gets [`@PlainBody`'s size limit](./PlainBody#size-limit): 100 kB on Express unless you register a text parser with another limit, and Fastify's `bodyLimit` there. Base64 makes the ciphertext about a third longer than the JSON it encrypts.
...
Any other value, such as a misspelled `"assertEqual"`, fails the build with a diagnostic listing these values, rather than falling back to a default.
