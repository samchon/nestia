...
description: Parse and validate `application/json` request bodies against pure TypeScript types — 20,000x faster than `class-validator` with 400 errors on schema mismatch.
---
...
`@TypedBody()` is not essential for [Swagger Documents](/docs/swagger) or [SDK Library](/docs/sdk) building.
...
`Nestia` will analyze your type (`IBbsArticle.IUpdate`), and writes optimal code for the target type, in the compilation level. If you click the "Compiled JavaScript File" tab of above, you can see the optimal validation code. 
...
For reference, such encryption spec is not supported in the [Swagger-UI](/docs/swagger). Instead, [SDK (Software Development Kit)](/docs/sdk) generated by `@nestia/sdk` supports it. Thus, you have to build and distribute the SDK library to the client developers when using such encryption decorators.




## Configuration
```json filename="tsconfig.json" showLineNumbers {8,9}
{
  "compilerOptions": {
    "strict": true,
    "plugins": [
...
Also, if you turn on the `llm` property, `@nestia/core` validates at compile time that the body type stays within the schema subset LLM function calling accepts.

`llm: true` forbids tuple types. `llm: { "strict": true }` additionally forbids `additionalProperties` (dynamic properties) and optional properties. Any violation is a compilation error.
