...
Also, supports [SDK (Software Development Kit)](/docs/sdk), so that you can easily develop the WebSocket client.
...
Also, `@WebSocketRoute.Param()` allows nullable like `number | null`, but undefinable type is not.
...
At first, type of `@WebSocketRoute.Query()` must be a pure **object type**. It does not allow union type. Also, nullable and undefinable types are not allowed, either. Note that, query parameter type must be a sole **object type** without any extra definition.

At next, type of properties must be **atomic**, or array of atomic type. In the atomic type case, the atomic type allows both nullable and undefinable types. However, mixed union atomic type like `string | number` or `"1" | "2" | 3` are not allowed. Also, the array type does not allow both nullable and undefinable types, either.
