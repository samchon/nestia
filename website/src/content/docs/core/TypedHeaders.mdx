...
`@nestia/sdk` still reads the declared type of a vanilla `@Headers()` parameter into the SDK and the Swagger document, but the request goes through without coercion or validation. A header object with a dynamic key, such as `Record<string, string>`, stays one parameter in Swagger, because its headers have no names to decompose into.
