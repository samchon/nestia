...
`@nestia/sdk` reads the declared type of `@TypedHeaders()` and vanilla `@Headers()` parameters alike into the SDK and the Swagger document; the vanilla decorator only skips coercion and validation. When Swagger decomposes a header object (the default), a dynamic key adds no parameter, because OpenAPI cannot list arbitrary headers: a `Record<string, string>` object adds none, and an object that mixes named headers with an index signature adds the named ones.
