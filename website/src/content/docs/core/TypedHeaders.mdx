...
description: Type-safe HTTP header decorator that parses request headers into a typed DTO and rejects invalid values with a 400 Bad Request via `typia.assert()`.
---
...
`@TypedHeaders()` is not essential for [Swagger Documents](/docs/swagger) or [SDK Library](/docs/sdk) building.
