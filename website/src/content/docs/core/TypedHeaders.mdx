...
`@nestia/sdk` reads the declared type of `@TypedHeaders()` and vanilla `@Headers()` parameters alike into the SDK and the Swagger document; the vanilla decorator only skips coercion and validation. A header object with a dynamic key, such as `Record<string, string>`, contributes only its known header names to Swagger, because OpenAPI cannot list arbitrary headers.
