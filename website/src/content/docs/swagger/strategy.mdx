...
If you don?셳 want that, you can adapt the second way that writing `@summary {string}` or `@title {string}` comment tag. In that case, the title value would not be contained in the description value. For reference, it is possible to fill the description property of the ILlmSchema by the comment tag `@description`, but you have to take care of the indentation like below.
...
You can utilize [type tags](https://typia.io/docs/validators/tags/#type-tags) (or [validator?셲 comment tags](https://typia.io/docs/validators/tags/#comment-tags)) to constructing special fields of JSON schema.

If you write any comment on a property, it would fill the IJsonSchema.description value. Also, there?셱e special comment tags only for JSON schema definition that are different with [validator?셲 comment tags](https://typia.io/docs/validators/tags/#comment-tags) like below.
...
Let?셲 see how those [type tags](https://typia.io/docs/validators/tags/#type-tags), comment tags and description comments are working with example code.
