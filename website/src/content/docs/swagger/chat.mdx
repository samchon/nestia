...
description: Turn a Nestia-generated Swagger document into an LLM function-calling agent with `@agentica` — skip the agent-graph plumbing and let the schema drive tool use.
---
...
<Callout type="info">
If you want an OpenAI-compatible hosted provider for this workflow, see the [Atlas Cloud provider guide](/docs/swagger/atlas-cloud).
</Callout>
...
The last package [`@samchon/openapi`](https://github.com/samchon/openapi) is a library for converting OpenAPI document to LLM function calling schema.
...
It will upgrade the NestJS backend server to be compatible with WebSocket protocol.
...
When a client connects to the server with `ws://localhost:3001/chat` like URL, Agentica made chatbot would be composed and Super AI chatbot conversation will be started.
