...
`NestiaSwaggerComposer` needs the `@nestia/sdk` transform active at build time. The auto-discovery in `ttsc` handles this on the `@next` line, but if you have an explicit `compilerOptions.plugins`, make sure the SDK transform is there:
...
```jsonc filename="tsconfig.json" copy
{
  "compilerOptions": {
    ...
    "plugins": [
      ...
      { "transform": "@nestia/sdk/lib/transform" },                // <-- required
...
If the plugin is missing or stale, `NestiaSwaggerComposer` finds zero routes and emits an empty document. The fix is always: re-enable the plugin, then rebuild.
...
- **[Recipes → Migrating from `@nestjs/swagger`](/docs/recipes/from-nestjs-swagger)** — mapping table.
