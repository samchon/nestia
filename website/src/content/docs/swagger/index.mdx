...
The `openapi` field accepts `"3.2"` (default), `"3.1"`, `"3.0"`, or `"2.0"`. Nestia composes the document at the newest version and downgrades on the way out — your type fidelity is highest at the newest version, but the document is still legal at older ones.

A Swagger 2.0 document holds less than an OpenAPI 3.x one, so with `openapi: "2.0"` the generator leaves out or narrows what 2.0 has no place for instead of failing: named examples and request body examples, the `x-nestia-encrypted` flag (the description keeps the encryption warning), a form body's own description, and the bodies of exceptions whose media type differs from the success body's. A form field taking several files, or a file or null, is listed as one file, optional unless the field requires at least one.
...
- **Response schemas** from the return type. `200` / `201` are inferred from the HTTP method. A route whose return type carries no value — `void`, `undefined`, `never` — still declares its status, with no response body attached; the same holds for a `@TypedException<void>()`.
...
Routes using vanilla `@Get` / `@Body` appear too, described from their declared types; they only lose runtime validation. To leave a route out of the document, tag the method `@internal` or `@hidden`, or `@ignore` to leave it out of every generator.
...
| `openapi`    | `"3.2" \| "3.1" \| "3.0" \| "2.0"`     | Defaults to `"3.2"`.                                           |
...
| `decompose`  | `boolean`                             | Defaults to `true`: one parameter per property of a query DTO. `false` keeps it one `style: form` parameter, except in Swagger 2.0. Headers DTOs are always decomposed. |
| `operationId`| `(props) => string`                   | Custom operationId generator.                                  |
| `beautify`   | `boolean`                             | Pretty-print the JSON output.                                  |
| `additional` | `boolean`                             | Adds `x-nestia-method`, `x-nestia-namespace` (the SDK accessor), and `x-nestia-jsDocTags` to each operation. |

Decomposed query and header properties marked with `?` produce parameters with `required: false`, including when `exactOptionalPropertyTypes` is enabled.

---

## See also

- **[Swagger → Strategy](./strategy)** — JSDoc descriptions, namespaces, function hiding, specialization.
