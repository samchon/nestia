...
description: Generate OpenAPI documents directly from NestJS controllers and pure TypeScript types — no `@ApiProperty` decorators required, powered by `NestiaSwaggerComposer`.
---
...
```
  ...
  <Tabs.Tab>
```jsonc filename="tsconfig.json" showLineNumbers
{
  "compilerOptions": {
    "strict": true,
    "plugins": [
...
When you want to build not only Swagger Document file, but also [SDK (Software Development Kit) library](/docs/sdk) and [automated E2E (End-to-End) test functions](/docs/e2e) at the same time, run `npx nestia all` command instead.
...
npx nestia swagger --config nestia3.config.ts --project tsconfig3.json
```
...
Swagger generator `@nestia/sdk` supports three type of comment tags for controller methods:
  ...
  - Labeling
    ...
    - `@tag {name} {description?}`: grouping with description
...
The 1st is publishing the `swagger.json` file in a public repo, and showing it through [`@nestia/editor`](/docs/swagger/editor) like below:
