...
description: Type-safe `multipart/form-data` body decorator for file uploads that casts and validates fields against a DTO — SDK-compatible replacement for `@UploadFile()`.
---
...
As you can see from the below code, `@TypedFormData.Body()` function is much easier and type safer than `@UploadFile()` of NestJS. Also, if you're considering the [SDK library](/docs/sdk) generation, only `@TypedFormData.Body()` is supported. Therefore, I recommend you to utilize `@TypedFormData.Body()` instead of the `@UploadFile()` function.

Of course, as every features of `nestia` does, you don't need to define any extra schema definition for the [Swagger Documents](/docs/swagger) generation. `@nestia/sdk` and `@TypedFormData.Body()` will do everything just by analyzing your TypeScript types and codes.
...
At first, type of `@TypedFormData.Body()` must be a pure **object type**. It does not allow union type. Also, nullable and undefinable types are not allowed, either. Note that, query parameter type must be a sole **object type** without any extra definition.

At next, type of properties must be **atomic**, `Blob`, `File` or array of them. In the atomic type case, the atomic type allows both nullable and undefinable types. However, mixed union atomic type like `string | number` or `"1" | "2" | 3` are not allowed. Also, the array type does not allow both nullable and undefinable types, either.
