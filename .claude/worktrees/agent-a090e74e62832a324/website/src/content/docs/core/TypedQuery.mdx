...
description: Type-safe query-string decorators that parse and validate URL parameters against a TypeScript DTO, plus `application/x-www-form-urlencoded` body support.
---
...
`@TypedQuery()` is not essential for [Swagger Documents](/docs/swagger) or [SDK Library](/docs/sdk) building.
...
At first, type of `@TypedQuery()` must be a pure **object type**. It does not allow union type. Also, nullable and undefinable types are not allowed, either. Note that, query parameter type must be a sole **object type** without any extra definition.
