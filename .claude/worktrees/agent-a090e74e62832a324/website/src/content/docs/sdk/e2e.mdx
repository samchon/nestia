...
description: Scaffold end-to-end test templates for every NestJS endpoint with `npx nestia e2e` — typed `IConnection`-driven functions you extend with real scenarios.
---
...
Then, `@nestia/sdk` will analyze your NestJS backend server code, and generate both [SDK (Software Development Kit) library](/docs/sdk) for client developers. Also, E2E test functions, utilizing the [SDK library](/docs/sdk), will be automatically generated for corresponding to every API functions.
...
When you've completed above configuration, just run `npx nestia e2e` command. Then, [SDK library](/docs/sdk) would be generated into the `$config.output` directory, and E2E test functions would be generated into the `$config.e2e` directory, following your `nestia.config.ts` option.
