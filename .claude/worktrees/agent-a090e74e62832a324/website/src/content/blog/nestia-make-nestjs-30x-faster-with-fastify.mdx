...
In previous article, I introduced my library `nestia`, making `NestJS` much easier and much faster. In the article, introducing performance enhancements, I had told that `nestia` can boost up validation speed maximum 20,000x faster (JSON serialization is 200x faster). 
...
### Reference
For reference, request body validation utilized an Array instance with length 100. If reduce the length to be 10, performance enhancement be halved (about 60%). Otherwise, as increase the length to be larger as, performance enhancement be dramatically increased.
...
The last benchmark is about composite performance, validating request body data and serialize JSON response data at the same time. As `nestia` had shown significant performance gaps, composite benchmark also shows significant performance gaps.
