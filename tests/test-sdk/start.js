...
const feature = (name) => {
  ...
  if (name.includes("error")) {
    try {
      ...
      if (fs.existsSync("src/test"))
        cp.execSync("npx ts-node -O '{\"module\":\"commonjs\",\"moduleResolution\":\"node\"}' src/test", { stdio: "ignore" });
    } catch {
      return;
    }
  ...
  if (name === "cli-project" || name === "cli-config-project") return;
    ...
    const test = (stdio) => cp.execSync("npx ts-node -O '{\"module\":\"commonjs\",\"moduleResolution\":\"node\"}' src/test", { stdio });
    for (let i = 0; i < 3; ++i)
      try {
        test("ignore");
        return;
