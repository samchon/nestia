...
/**
 * Verifies @TypedHeaders round-trips correctly when the @nestia/core plugin is
 * configured with `validate: "assert"`.
 *
 ...
 * is-programmer, `validate*` → validate-programmer, everything else (assert
 * family) → assert-programmer. So this fixture and the base `headers` fixture
 * both route through `HttpAssertHeadersProgrammer`. The pair pins that the
 * tsconfig validate option doesn't accidentally change observable header
 * behavior across modes in the assert family.
 *
