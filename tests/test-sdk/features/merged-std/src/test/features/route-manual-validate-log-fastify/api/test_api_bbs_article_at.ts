...
import api from "@api";
import { IBbsArticle } from "../../../../features/route-manual-validate-log-fastify/api/structures/IBbsArticle";
...
export const test_api_bbs_article_at = async (
  ...
  const id: string = v4();
  const article: IBbsArticle = await api.functional.route_manual_validate_log_fastify.bbs.articles.at(
    connection,
    id,
  );
