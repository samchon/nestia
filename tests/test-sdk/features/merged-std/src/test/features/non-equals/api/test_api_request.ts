...
export const test_api_request = async (
  ...
  const output: IRequestDto = await api.functional.non_equals.request(connection, input);
  TestValidator.equals("DTO", input, output);

  const surplus: IRequestDto = await api.functional.non_equals.request(connection, input);
