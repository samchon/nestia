...
export const test_api_param_date = async (
  ...
  const date = random();
  const value = await api.functional.param_validate.param.date(connection, date);
  TestValidator.equals("date", date, value);

  await TestValidator.error("null", () =>
    api.functional.param_validate.param.date(connection, null!),
  );
  await TestValidator.error("invalid", () =>
    api.functional.param_validate.param.date(connection, "20140102"),
  );
};
