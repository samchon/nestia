...
`stage`가 없으면 `"transform"`이다. 현재 `ttsc` loader는 `"output"` stage를 제거된 stage로 보고 명시적으로 거부한다. `transformSource`나 `transformOutput` 같은 JS hook도 거부된다.
...
- scratch copy에서 `node_modules`, `.git`, `.ttsc` directory와 `go.work`, `go.work.sum` generated workspace file을 제외한다.
