...
- **One `test-sdk` feature:** `pnpm --filter ./tests/test-sdk start -- --only <substring>`, which runs only the features whose name contains that substring. `--from <name>` resumes lexicographically, `--shard <index>/<count>` (or `TEST_SDK_SHARD`) runs one of `count` disjoint slices of the selected features, as the CI `sdk` jobs do, and `TEST_SDK_SKIP_BUILD=1` reuses the current package builds.
