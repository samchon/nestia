...
7. Record every lead and critic disposition in the campaign knowledge base. Whenever the round produced a candidate, replace the team and run another complete repository-wide round against the same commit; only an empty round ends the discovery phase. When the user authorized end-to-end implementation, implement every accepted issue after that empty round, freeze the new integrated commit, and run the same loop again. Otherwise stop at the authorized phase without mutation and report the campaign as active. A prior finding is not permission to narrow the next round to its owner or neighbors.
