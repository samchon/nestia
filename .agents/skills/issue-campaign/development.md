...
After every merged cycle, return to the parent skill's Discover Issues phase and run its full-scope round loop again against one frozen integrated commit, repeating rounds until one adds nothing already recorded. Earlier rounds are not coverage, and a surviving issue or recent implementation never permits a residual-only review.
