...
- `packages/*`: the eight published packages. The shared Go plugin lives under `packages/core/native` (module `github.com/samchon/nestia/packages/core/native`, with `cmd/ttsc-nestia` and the 13-file `transform/` tree); the SDK contributor lives under `packages/sdk/native/sdk`. Both `native/go.work` files carry the same fifteen `replace` directives: fourteen redirect the `github.com/microsoft/typescript-go/shim/*` modules to a pinned `github.com/samchon/ttsc` pseudo-version, and the fifteenth redirects `github.com/samchon/ttsc/packages/ttsc` itself.
- `packages/core/test` and `packages/sdk/test`: the Go test modules, each its own module — core's replaces `../native`, the SDK's replaces both `../native` and `../../core/native`. All 80 tracked `*_test.go` files live here; `native/` itself carries none.
...
Release-time commands (most contributors skip these): `pnpm package:rc`, `pnpm package:next`, `pnpm package:latest`, and `pnpm release`. Every `package:*` command first runs `package:prepare`, which builds all packages (the `@nestia/migrate` build regenerates its template bundles first) and copies the root README into each of them, and only then publishes or packs with `--ignore-scripts`; no package has a `prepack` hook and no lifecycle script runs during publish, so any failure aborts before anything is uploaded rather than after some packages already went out. `pnpm package:tgz` stages local tarballs in `deploy/tarballs/`, which the website build then installs. See `.agents/skills/pull-request/SKILL.md` for the remote delivery flow.
