...
Release-time commands (most contributors skip these): `pnpm package:rc`, `pnpm package:next`, `pnpm package:latest`, and `pnpm release`. Every `package:*` command first runs `package:prepare`, which regenerates the `@nestia/migrate` template bundles, builds all packages, and copies the root README into each of them, and only then publishes or packs with `--ignore-scripts`; no package has a `prepack` hook and no lifecycle script runs during publish, so any failure aborts before anything is uploaded rather than after some packages already went out. `pnpm package:tgz` stages local tarballs in `deploy/tarballs/`, which the website build then installs. See `.agents/skills/pull-request/SKILL.md` for the remote delivery flow.
