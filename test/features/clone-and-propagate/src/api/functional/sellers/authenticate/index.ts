...
export async function join(
  ...
  const output: join.Output = !!connection.simulate
    ? await join.simulate(connection, input)
...
export async function login(
  ...
  const output: login.Output = !!connection.simulate
    ? await login.simulate(connection, input)
    : await EncryptedFetcher.propagate(
        {
          ...connection,
          headers: {
...
export namespace exit {
  ...
  export const simulate = async (_connection: IConnection): Promise<Output> => {
    return {
      success: true,
      status: 200,
      headers: {
